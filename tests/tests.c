...
void walk_tree(const cidr_node *node, int uml_type)
{
    ...
    if (uml_type) {
        ...
        if (node->l) {
            ...
            printf("agent \"%s%s\"\n", child_node && !child_node->data ? "(v) " : "", cidr_child);
            char skip_str[32];
        ...
        printf("node: %3s %-18s l: %3s %-18s r: %3s %-18s\n",
            !node->data ? "(v)" : "",
            cidr,
            node->l && !node->l->data ? "(v)" : "",
            node->l ? cidr_child_l : "",
            node->r && !node->r->data ? "(v)" : "",
            node->r ? cidr_child_r : "");
    }
...
int test_cidr_add_node()
{
    ...
    for (int i = 0; i < array_size; i++) {
        ipmask_parse(add_node_test_cases[i].cidr, &ip, &bits);
        cidr_add_node(root_tree, &ip, bits, &add_node_test_cases[i]);
    }
    node = root_tree->ipv4;
    printf("ipv4 tree:\n");
    walk_tree(node, 1);
    ...
    for (int i = 0; i < array_size; i++) {
        ipmask_parse(add_node_test_cases[i].cidr, &ip, &bits);
        node = _cidr_find_exact_node(root_tree, &ip, bits);
    ...
    for (int i = 0; i < array_size; i++) {
        ipmask_parse(find_node_test_cases[i].cidr, &ip, &bits);
        node = cidr_search_best(root_tree, &ip, bits);
        cidr = node ? ircd_ntocidrmask(&node->ip, node->bits) : 0;
    ...
    for (int i = 0; i < array_size; i++) {
        ipmask_parse(rem_node_test_cases[i].cidr, &ip, &bits);
        cidr_rem_node_by_cidr(root_tree, &ip, bits);
    }
    node = root_tree->ipv4;
